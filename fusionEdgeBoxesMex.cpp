...
typedef struct { int c, r, w, h; float s; float ss[2]; } Box;
...
public:
  ...
  // Fusion functions.
  void fusion_genAllBoxes(Boxes& boxes);
  void fusion_scoreBox(vector<EdgeBoxGenerator>& workers,
                       vector<float>& weights,
                       Box& box);
  void fusion_refineBox(vector<EdgeBoxGenerator>& workers,
                        vector<float>& weights,
                        Box& box);
  void fusion_scoreAllBoxes(vector<EdgeBoxGenerator>& workers,
                            vector<float>& weights,
                            Boxes& boxes);
};
...
////////////////////////////////////////////////////////////////////////////////
// Fusion functions. We do not use static functions because a few shared
// parameters are stored in objects.

// Generate a set of boxes w/ different size, aspect ratio and scale.
void EdgeBoxGenerator::fusion_genAllBoxes(Boxes& boxes)
{
    // get list of all boxes roughly distributed in grid
    boxes.resize(0); int arRad, scNum; float minSize=sqrt(_minBoxArea);
    arRad = int(log(_maxAspectRatio)/log(_arStep*_arStep));
...
// Calculate fusion score for a box.
void EdgeBoxGenerator::fusion_scoreBox(vector<EdgeBoxGenerator>& workers,
                                       vector<float>& weights,
                                       Box& box)
{
    int num_worker = workers.size();
    
    vector<float> scores(workers.size(), 0);
    for(int i = 0; i < num_worker; ++i)
    {
        // Although scoreBox function will modify the box dimension, once
        // modified, the box dimension won't be changed in next function call.
        workers[i].scoreBox(box);
        // If any worker has a zero score, we say there is no motion object.
        if( !box.s )
        {
            box.s = 0;
            return;
    ...
    box.s = 0;
    for(int i = 0; i < num_worker; ++i)
        box.s += weights[i] * scores[i];

    // Store each worker's score.
    for(int i = 0; i < num_worker; ++i)
        box.ss[i] = scores[i];
}

// Refine a box using fusion score.
void EdgeBoxGenerator::fusion_refineBox(vector<EdgeBoxGenerator>& workers,
                                        vector<float>& weights,
                                        Box& box)
{
    int rStep = int(box.h*_rcStepRatio);
    ...
    while( 1 ) {
        ...
        rStep=max(1,rStep); cStep=max(1,cStep); Box B;
        // search over r start
        B=box; B.r=box.r-rStep; B.h=B.h+rStep; fusion_scoreBox(workers, weights, B);
        if(B.s<=box.s) { B=box; B.r=box.r+rStep; B.h=B.h-rStep; fusion_scoreBox(workers, weights, B); }
        if(B.s>box.s) box=B;
        // search over r end
        B=box; B.h=B.h+rStep; fusion_scoreBox(workers, weights, B);
        if(B.s<=box.s) { B=box; B.h=B.h-rStep; fusion_scoreBox(workers, weights, B); }
        if(B.s>box.s) box=B;
        // search over c start
        B=box; B.c=box.c-cStep; B.w=B.w+cStep; fusion_scoreBox(workers, weights, B);
        if(B.s<=box.s) { B=box; B.c=box.c+cStep; B.w=B.w-cStep; fusion_scoreBox(workers, weights, B); }
        if(B.s>box.s) box=B;
        // search over c end
        B=box; B.w=B.w+cStep; fusion_scoreBox(workers, weights, B);
        if(B.s<=box.s) { B=box; B.w=B.w-cStep; fusion_scoreBox(workers, weights, B); }
        if(B.s>box.s) box=B;
    }
}

// Calculate fusion scores for a set of boxes, moreover, do the NMS.
void EdgeBoxGenerator::fusion_scoreAllBoxes(vector<EdgeBoxGenerator>& workers,
                                            vector<float>& weights,
                                            Boxes& boxes)
{
    // score all boxes, refine top candidates, perform nms
    ...
    for( i=0; i<m; i++ )
    {
        fusion_scoreBox(workers, weights, boxes[i]);
        if( !boxes[i].s ) continue; k++;
        fusion_refineBox(workers, weights, boxes[i]);
    }
    sort(boxes.rbegin(),boxes.rend(),boxesCompare);
    boxes.resize(k); boxesNms(boxes,_beta,_maxBoxes);
    mexPrintf("%d %d %d\n", m, k, boxes.size());
...
// Matlab entry point: bbs = mex( objE, objO, floE, floO, prm1, prm2, ... )
void mexFunction( int nl, mxArray *pl[], int nr, const mxArray *pr[] )
{
  // check and get inputs
  if(nr != 18) mexErrMsgTxt("Invalid number of parameters.");
  if(nl > 4) mexErrMsgTxt("At most two outputs expected.");
  for(int i=0; i<4; ++i)
      if(mxGetClassID(pr[i]) != mxSINGLE_CLASS)
          mexErrMsgTxt("Input matrix must be a float*");

  vector<arrayf> E(2);
  vector<arrayf> O(2);

  arrayf& objE = E[0]; objE._x = (float*) mxGetData(pr[0]);
  arrayf& objO = O[0]; objO._x = (float*) mxGetData(pr[1]);

  arrayf& floE = E[1]; floE._x = (float*) mxGetData(pr[2]);
  arrayf& floO = O[1]; floO._x = (float*) mxGetData(pr[3]);

  int h = (int) mxGetM(pr[0]); objE._h = objO._h = h; floE._h = floO._h = h;
  int w = (int) mxGetN(pr[0]); objE._w = objO._w = w; floE._w = floO._w = w;
  ...
  if(nl>1)
  {
      ...
      {
          pl[i+1] = mxCreateNumericArray(3, ds, mxSINGLE_CLASS, mxREAL);
          V[i]._x = (float*)mxGetData(pl[i+1]);
          V[i]._h = h;
          V[i]._w = w;
      }
  }
  ...
  vector<EdgeBoxGenerator> workers(2);
  for(int i=0; i<workers.size(); ++i)
  {
      EdgeBoxGenerator& edgeBoxGen = workers[i];

      edgeBoxGen._alpha = float(mxGetScalar(pr[4]));
      edgeBoxGen._beta = float(mxGetScalar(pr[5]));
      edgeBoxGen._minScore = float(mxGetScalar(pr[6]));
      edgeBoxGen._maxBoxes = int(mxGetScalar(pr[7]));

      edgeBoxGen._edgeMinMag = float(mxGetScalar(pr[8 + i]));

      edgeBoxGen._edgeMergeThr = float(mxGetScalar(pr[10]));
      edgeBoxGen._clusterMinMag = float(mxGetScalar(pr[11]));
      edgeBoxGen._maxAspectRatio = float(mxGetScalar(pr[12]));
      edgeBoxGen._minBoxArea = float(mxGetScalar(pr[13]));
      edgeBoxGen._gamma = float(mxGetScalar(pr[14]));
      edgeBoxGen._kappa = float(mxGetScalar(pr[15]));

      edgeBoxGen.clusterEdges(E[i], O[i], V[i]);
      edgeBoxGen.prepDataStructs(E[i]);
  }

  vector<float> weights(workers.size());
  for(int i=0; i<weights.size(); ++i)
      weights[i] = float(mxGetScalar(pr[16+i]));

  Boxes boxes;
  workers[0].fusion_genAllBoxes(boxes);
  workers[0].fusion_scoreAllBoxes(workers, weights, boxes);

  // create output bbs and output to Matlab
  int n = (int) boxes.size();
  pl[0] = mxCreateNumericMatrix(n,5+2,mxSINGLE_CLASS,mxREAL);
  ...
  for(int i=0; i<n; i++) {
    ...
    bbs[ i + 4*n ] = boxes[i].s;
    bbs[ i + 5*n ] = boxes[i].ss[0];
    bbs[ i + 6*n ] = boxes[i].ss[1];
  }
}
